...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\labmp-psychopy\\Desktop\\replay_pilot\\2.0 learn_probe_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions_main = visual.TextStim(win=win, name='instructions_main',
        text="Cette partie de l'expérience se divise en deux phases, répétées trois fois.\nDans un premier temps, vous devrez apprendre des liens entre des paires d'images présentées, sans effectuer de réponse active.\nEnsuite, lors de la deuxième phase, une image vous sera présentée, puis vous devrez indiquer si l’image suivante correspond à la prochaine dans la séquence apprise.\n\n\n\n",
        font='Arial',
    ...
    condition_probe = f"conditions_probe/probe_conditions_{last_three_digits}.csv"
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Bloc terminé, utilisez la barre d’espace pour continuer.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    instr_rest_state = visual.TextStim(win=win, name='instr_rest_state',
        text="Il vous est demandé de rester devant l'écran, les yeux ouverts, sans rien faire pendant les prochaines minutes",
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "rest_state" ---
    ...
    for thisBlocks_loop in blocks_loop:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        text.setText('Des images vous seront présentées par paires. Vous devez apprendre les liens entre les images sans donner de réponse.')
        ...
        for thisLearn_image in learn_images:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from eeg_image_1
            wait_sent = 0
            ...
            while continueRoutine and routineTimer.getTime() < 1.5:
                ...
                frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
                # update/draw components on each frame
                # Run 'Each Frame' code from eeg_image_1
                if first_of_pair.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
            ...
            # Run 'End Routine' code from eeg_image_1
            port.write([0x00])
            ...
            while continueRoutine:
                ...
                if break_random.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from eeg_image_2
            wait_sent = 0
            
            #this trigger corresponds to word trigger in code 1
            
            # Run 'Begin Routine' code from photo_sensor_2
            photodiode_box = visual.Rect(
                win=win,
                width=18,
                height=25,
            ...
            while continueRoutine and routineTimer.getTime() < 1.5:
                ...
                # Run 'Each Frame' code from eeg_image_2
                if second_of_pair.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
                    win.callOnFlip(port.write, data=b'\x10') #Send the trigger, synced to the screen refresh
                    #win.callOnFlip(eci_client.send_event, event_type = 'wait') #Send the trigger, synced to the screen refresh
                    wait_sent = 1 #The wait has now been sent, so we set this to true to avoid a trigger being sent on each frame
                
                # Run 'Each Frame' code from photo_sensor_2
                if second_of_pair.status == STARTED:
                    photodiode_box.autoDraw = True
                else:
                    photodiode_box.autoDraw = False
            ...
            # Run 'End Routine' code from eeg_image_2
            port.write([0x00])
            # Run 'End Routine' code from photo_sensor_2
            photodiode_box.autoDraw = False
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        ...
        for thisProbe_loop in probe_loop:
            ...
            while continueRoutine and routineTimer.getTime() < 0.3:
                ...
                if image_load.status == NOT_STARTED and t >= 0.0-frameTolerance:
                    ...
                    prob_test_img.setImage(probe_img)
                    # Component updates done
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from eeg_prob_1
            wait_sent = 0
            # Run 'Begin Routine' code from photo_sensor_3
            photodiode_box = visual.Rect(
                win=win,
                width=18,
                height=25,
            ...
            while continueRoutine and routineTimer.getTime() < 4.0:
                ...
                # Run 'Each Frame' code from eeg_prob_1
                
                #Check trigger 5
                
                
                if probe_cue_1.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
                    win.callOnFlip(port.write, data=b'\x04') #Send the trigger, synced to the screen refresh
                    #win.callOnFlip(eci_client.send_event, event_type = 'wait') #Send the trigger, synced to the screen refresh
                    wait_sent = 1 #The wait has now been sent, so we set this to true to avoid a trigger being sent on each frame
                
                
                # Run 'Each Frame' code from photo_sensor_3
                if probe_cue_1.status == STARTED:
                    photodiode_box.autoDraw = True
                else:
                    photodiode_box.autoDraw = False
            ...
            # Run 'End Routine' code from eeg_prob_1
            port.write([0x00])
            # Run 'End Routine' code from photo_sensor_3
            photodiode_box.autoDraw = False
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            ...
            # Run 'Begin Routine' code from eeg_blank_3
            wait_sent = 0
            ...
            while continueRoutine:
                ...
                # Run 'Each Frame' code from eeg_blank_3
            ...
            # Run 'End Routine' code from eeg_blank_3
            port.write([0x00])
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from eeg_probe_2
            wait_sent = 0
            # Run 'Begin Routine' code from photo_sensor_4
            photodiode_box = visual.Rect(
                win=win,
                width=18,
                height=25,
            ...
            while continueRoutine and routineTimer.getTime() < 2.0:
                ...
                # Run 'Each Frame' code from eeg_probe_2
                #check trigger 6
                
                if prob_test_img.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
                    win.callOnFlip(port.write, data=b'\x20') #Send the trigger, synced to the screen refresh
                    #win.callOnFlip(eci_client.send_event, event_type = 'wait') #Send the trigger, synced to the screen refresh
                    wait_sent = 1 #The wait has now been sent, so we set this to true to avoid a trigger being sent on each frame
                
                
                # Run 'Each Frame' code from photo_sensor_4
                if prob_test_img.status == STARTED:
                    photodiode_box.autoDraw = True
                else:
                    photodiode_box.autoDraw = False
            ...
            # Run 'End Routine' code from eeg_probe_2
            port.write([0x00])
            # Run 'End Routine' code from photo_sensor_4
            photodiode_box.autoDraw = False
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            ...
            while continueRoutine and routineTimer.getTime() < 4.0:
                ...
                if text_4.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
            ...
            text_2.setText('')
            # Run 'Begin Routine' code from eeg_blank_2
            wait_sent = 0
            ...
            while continueRoutine:
                ...
                # Run 'Each Frame' code from eeg_blank_2
                if break_random.status == STARTED and wait_sent == 0: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
                    win.callOnFlip(port.write, data=b'\x40') #Send the trigger, synced to the screen refresh
                    #win.callOnFlip(eci_client.send_event, event_type = 'wait') #Send the trigger, synced to the screen refresh
                    wait_sent = 1 #The wait has now been sent, so we set this to true to avoid a trigger being sent on each frame
            ...
            thisExp.addData('probe_image', probe_img)
            ...
            # Run 'End Routine' code from eeg_blank_2
            port.write([0x00])
    ...
    # create an object to store info about Routine rest_instr
    rest_instr = data.Routine(
        name='rest_instr',
        components=[instr_rest_state, key_resp_3],
    )
    rest_instr.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    # store start times for rest_instr
    rest_instr.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    rest_instr.tStart = globalClock.getTime(format='float')
    rest_instr.status = STARTED
    thisExp.addData('rest_instr.started', rest_instr.tStart)
    rest_instr.maxDuration = None
    # keep track of which components have finished
    rest_instrComponents = rest_instr.components
    for thisComponent in rest_instr.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    rest_instr.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # *instr_rest_state* updates
        
        # if instr_rest_state is starting this frame...
        if instr_rest_state.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instr_rest_state.frameNStart = frameN  # exact frame index
            instr_rest_state.tStart = t  # local t and not account for scr refresh
            instr_rest_state.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instr_rest_state, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instr_rest_state.started')
            # update status
            instr_rest_state.status = STARTED
            instr_rest_state.setAutoDraw(True)
        
        # if instr_rest_state is active this frame...
        if instr_rest_state.status == STARTED:
            # update params
            pass
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            rest_instr.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in rest_instr.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in rest_instr.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for rest_instr
    rest_instr.tStop = globalClock.getTime(format='float')
    rest_instr.tStopRefresh = tThisFlipGlobal
    thisExp.addData('rest_instr.stopped', rest_instr.tStop)
    # check responses
    ...
    # --- Prepare to start Routine "rest_state" ---
    # create an object to store info about Routine rest_state
    rest_state = data.Routine(
        name='rest_state',
        components=[text_5],
    )
    rest_state.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from eeg_fix_2
    wait_sent = 0
    # store start times for rest_state
    rest_state.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    rest_state.tStart = globalClock.getTime(format='float')
    rest_state.status = STARTED
    thisExp.addData('rest_state.started', rest_state.tStart)
    rest_state.maxDuration = None
    # keep track of which components have finished
    rest_stateComponents = rest_state.components
    for thisComponent in rest_state.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "rest_state" ---
    rest_state.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 300.0:
        ...
        if text_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_5.tStartRefresh + 300-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # Run 'Each Frame' code from eeg_fix_2
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            rest_state.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in rest_state.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "rest_state" ---
    for thisComponent in rest_state.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for rest_state
    rest_state.tStop = globalClock.getTime(format='float')
    rest_state.tStopRefresh = tThisFlipGlobal
    thisExp.addData('rest_state.stopped', rest_state.tStop)
    # Run 'End Routine' code from eeg_fix_2
    port.write([0x00])
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if rest_state.maxDurationReached:
        routineTimer.addTime(-rest_state.maxDuration)
    elif rest_state.forceEnded:
        routineTimer.reset()
    else:
